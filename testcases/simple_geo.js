...
/* Testcase: Geo.geoJSON.nearSphere.2dsphere.find100withFilter */
var ops = [];
for( var i = 0; i < 5; i++) {
    for( var j = 0; j < 5; j++) {
        ops.push({
...
/* 
 * Setup: create map with 2dsphere index
 * Test: Geo.geoJSON.nearSphere.2dsphere.find100withFilter
 *     - to test nearSphere query with geoJSON format, 
 *     - with 2dsphere index
 *     - only return one in 100 results
 *     - limit to 100 docs
 *     - should scan ~3,000 documents
 */
tests.push( { name: "Geo.geoJSON.nearSphere.2dsphere.find100WithFilter",
              tags: ['geo','core','indexed'],
              pre: function( collection ) { 
                  generateGridMap(collection, 
                          x_min, y_min, x_max, y_max, "2dsphere", false);
...
/* Testcase: Geo.near.2d.find100WithFilter */
ops = [];
for( var i = 0; i < 5; i++) {
    for( var j = 0; j < 5; j++) {
        ops.push({
...
/*
 * Setup: generate map with 2d index
 * Test: Geo.near.2d.find100WithFilter
 *     - to test $near query with legacy format, 
 *     - with 2d index
 *     - limit to 100 doc
 *     - should scan ~3,000 documents
 */
tests.push( { name: "Geo.near.2d.find100WithFilter",
              tags: ['geo','core','indexed'],
              pre: function( collection ) { 
                  generateGridMap(collection, 
                          x_min, y_min, x_max, y_max, "2d", true);
