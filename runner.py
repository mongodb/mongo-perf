...
def parse_options():
    ...
    optparser.add_option('--mongod', dest='mongod', help='path to mongod executable', type='string', default='mongod')
...
def run_benchmark(opts):
    ...
    try:
        benchmark = subprocess.Popen(['./benchmark', opts.port, opts.iterations, \
        ...
        print >> sys.stderr, "Could not start complete benchmark tests", sys.exc_info()[0]
        retval = cleanup()
        sys.exit(retval)
